...
from jfscripts.list_files import (
    _list_files_all,  # type: ignore
    _list_files_filter,  # type: ignore
    _split_glob,  # type: ignore
    common_path,
    doc_examples,
    is_glob,
    list_files,
...
class TestFunctionIsGlob:
    def test_asterisk_extension(self) -> None:
        assert is_glob("*.mscx")

    def test_asterisk(self) -> None:
        assert is_glob("l*l")

    def test_quotation_mark(self) -> None:
        assert is_glob("l?l")

    def test_seq(self) -> None:
        assert is_glob("l[ol]")

    def test_not_seq(self) -> None:
        assert is_glob("l[!ol]")
...
class TestFunctionListFilesAll:
    ...
    @mock.patch("os.walk")
    def test_only_dirs(self, os_walk: mock.Mock) -> None:
    ...
    @mock.patch("os.walk")
    def test_files_and_dirs(self, os_walk: mock.Mock) -> None:
...
class TestFunctionListFilesFilter:
    @mock.patch("os.walk")
    def test_simple(self, os_walk: mock.Mock) -> None:
    ...
    @mock.patch("os.walk")
    def test_no_match(self, os_walk: mock.Mock) -> None:
        os_walk.return_value = (("a", ("a", "b"), ("a.jpg", "b.xml")),)
        result = _list_files_filter("a", "*.txt")
        assert result == []

    @mock.patch("os.walk")
    def test_glob_middle_position(self, os_walk: mock.Mock) -> None:
    ...
    @mock.patch("os.walk")
    def test_glob_character_seq(self, os_walk: mock.Mock) -> None:
...
class TestFunctionListFiles:
    ...
    @mock.patch("os.walk")
    def test_glob(self, os_walk: mock.Mock) -> None:
    ...
    @mock.patch("os.walk")
    def test_glob_no_match(self, os_walk: mock.Mock) -> None:
        os_walk.return_value = (("/data", (), ("a.txt", "b.txt")),)
        assert list_files(["/data/*.py"]) == []

    @mock.patch("os.path.isdir")
    @mock.patch("os.walk")
    def test_default_glob(self, os_walk: mock.Mock, os_path_isdir: mock.Mock) -> None:
        os_walk.return_value = (("/data", (), ("a.txt", "b.txt")),)
...
class TestFunctionArgparseExamples:
    ...
    def test_without_indent_spaces(self) -> None:
        result = (
            "test.py a.txt\n"
    ...
    def test_with_indent_spaces(self) -> None:
        result = (
            "    test.py a.txt\n"
            "    test.py a.txt b.txt c.txt\n"
            "    test.py (asterisk).txt\n"
...
class TestIntegration:
    def test_command_line_interface(self) -> None:
        assert is_executable("list_files")
