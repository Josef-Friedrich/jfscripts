...
class TestCheckBin(TestCase):
    ...
    def test_check_dependencies_nonexistent(self):
        with mock.patch("shutil.which") as mock_which:
            ...
            self.assertEqual(
                str(error.exception), "Some commands are not installed: lol"
            )
    ...
    def test_check_dependencies_nonexistent_multiple(self):
        with mock.patch("shutil.which") as mock_which:
            ...
            self.assertEqual(
                str(error.exception), "Some commands are not installed: lol, troll"
            )
    ...
    def test_check_dependencies_nonexistent_multiple_with_description(self):
        with mock.patch("shutil.which") as mock_which:
            ...
            self.assertEqual(
                str(error.exception),
                "Some commands are not installed: lol (apt " "install lol), troll",
            )
